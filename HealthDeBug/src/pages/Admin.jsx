...
const Admin = () => {
  ...
  useEffect(()=>{
    const fetchdata = async ()=>{
        try{
            const response = await axios.get(`https://hackthon-1.onrender.com/api/v1/doctor/getonedoctor/66127d65fd10ce87ef16de78`);
      ...
      <div className="col-span-2 pt-8 pl-2">
        ...
        <div className="flex flex-col space-y-4 text-1xl max-md:text-sm">
          ...
          <div className="flex flex-col w-full">
            ...
            <textarea onChange={(e)=>{setPost(e.targate.value)}} className=" border rounded-lg p-2 w-60" placeholder="status..."/>
            <div className="mt-1">
                {/* api post */}
                <button onClick={async ()=>{
                    await axios.put("https://hackthon-1.onrender.com/api/v1/doctor/status",{
                        status: post
                    },{
                    headers:{
                ...
                }} className="border rounded-lg p-2 hover:bg-slate-200 active:border-black">Post</button>
            </div>
          </div>
